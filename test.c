...
int main(int argc, char **argv) {
	...
	bitstream_reader_t rd = {};
	rd.stream = &bs;

	{
		ASSERT_EQ(read_bits(&rd, 1), 0b1);
		ASSERT_EQ(read_bits(&rd, 4), 0b1010);
		ASSERT_EQ(read_bits(&rd, 3), 0b101);
		ASSERT_EQ(read_bits(&rd, 5), 0b11011);
		ASSERT_EQ(read_bits(&rd, 5), 0b10110);
		ASSERT_EQ(read_bits(&rd, 20), 0xABCDE);
		ASSERT_EQ(read_bits(&rd, 1), 0);
	}
	...
	printf("input size: %lu; compressed size: %lu; rate: %f\n", rsz, esz, (float)esz/(float)rsz);
